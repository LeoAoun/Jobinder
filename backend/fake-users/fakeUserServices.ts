const jardineiroServices: string[] = [
  "Especialista em poda de formação e condução, aplicando técnicas adequadas a cada espécie.",
  "Experiência em manejo de sistemas de irrigação automatizados para jardins residenciais e comerciais.",
  "Realizo adubação orgânica e química balanceada, de acordo com a necessidade do solo e das plantas.",
  "Profissional em controle fitossanitário, utilizando métodos biológicos e preventivos.",
  "Projetos de paisagismo com análise de solo, escolha de espécies adequadas ao microclima local.",
  "Instalação e manutenção de gramados esportivos, com técnicas de nivelamento e aeração do solo.",
  "Executo serviços de transplante de árvores de grande porte, com garantia de adaptação.",
  "Aplicação de técnicas de compostagem e manejo sustentável de resíduos orgânicos.",
  "Desenvolvo sistemas de drenagem para evitar erosão e encharcamento em áreas verdes.",
  "Capacitado em manutenção e afiação de ferramentas de jardinagem para maior eficiência.",
];

const marceneiroServices: string[] = [
  "Faço móveis sob medida com precisão e acabamento de alta qualidade.",
  "Especialista em restauração de móveis antigos, devolvendo vida às peças.",
  "Trabalho com cortes e encaixes perfeitos para móveis personalizados.",
  "Experiência em aplicação de verniz, seladora e outros acabamentos.",
  "Desenvolvo móveis planejados que otimizam seu espaço.",
  "Montagem e instalação de móveis residenciais e comerciais.",
  "Crio peças decorativas com detalhes personalizados em madeira.",
  "Projetos com madeiras sustentáveis e certificadas.",
  "Faço reparos e ajustes em móveis danificados.",
  "Leitura de projetos técnicos e execução fiel das ideias.",
];

const pintorServices: string[] = [
  "Especialista em pintura residencial interna e externa, com acabamento de alta qualidade.",
  "Aplicação de texturas e efeitos decorativos, como grafiato e marmorato.",
  "Experiência em pintura automotiva, com foco em polimento e retoques.",
  "Preparação de superfícies: lixamento, massa corrida e impermeabilização.",
  "Realizo pintura industrial com tintas resistentes a altas temperaturas.",
  "Executo serviços de pintura com pistola, rolo ou pincel, conforme a necessidade.",
  "Trabalho com pintura epóxi para pisos e áreas industriais.",
  "Faço restauração de fachadas com tratamento anticorrosivo.",
  "Aplicação de vernizes e seladoras em madeiras para proteção e brilho.",
  "Capacitado em pintura de paredes, portas e móveis com acabamento detalhado.",
];

const DJServices: string[] = [
  "Especialista em mixagens ao vivo, criando transições suaves entre diferentes estilos musicais.",
  "Experiência em eventos, festas e casamentos, com repertório personalizado.",
  "Domínio em softwares como Serato, Traktor e Rekordbox para performances precisas.",
  "Crio sets exclusivos com base no perfil do público e na vibe do evento.",
  "Capacitado em produção de remixes e mashups para hits populares.",
  "Habilidade em controlar sistemas de som e iluminação para shows.",
  "Realizo performances com vinil e controladoras digitais.",
  "Foco em sonorização de ambientes, ajustando o som para máxima qualidade.",
  "Experiência em música eletrônica, hip-hop, pop e outros estilos.",
  "Domínio de técnicas de scratching e beatmatching para apresentações dinâmicas.",
];

const advocaciaServices: string[] = [
  "Especialista em Direito Civil, com experiência em contratos e resolução de disputas.",
  "Atuo em Direito Trabalhista, defendendo os direitos de empresas e trabalhadores.",
  "Experiência em Direito Penal, com foco em defesa e assistência jurídica.",
  "Consultoria jurídica em Direito Empresarial, incluindo abertura e estruturação de empresas.",
  "Advogado com foco em Direito de Família: divórcios, guarda e inventários.",
  "Atuo em processos de recuperação judicial e falência.",
  "Especialista em Direito Tributário, com assessoria para otimização fiscal.",
  "Realizo análise e elaboração de contratos para pessoas físicas e jurídicas.",
  "Experiência em Direito do Consumidor, protegendo os interesses de clientes e empresas.",
  "Atendo causas relacionadas a propriedade intelectual, marcas e patentes.",
];

const personalServices: string[] = [
  "Especialista em musculação e hipertrofia, focado em alcançar resultados de força e definição.",
  "Atuo com treinamento funcional, promovendo equilíbrio, mobilidade e qualidade de vida.",
  "Experiência em preparação física para atletas amadores e profissionais.",
  "Consultoria em emagrecimento saudável, com treinos personalizados e acompanhamento contínuo.",
  "Personal Trainer para terceira idade, priorizando saúde, mobilidade e prevenção de lesões.",
  "Especialista em treinos de alta intensidade, otimizando o tempo de treino para resultados rápidos.",
  "Planejo treinos ao ar livre, aproveitando parques e espaços públicos para variações dinâmicas.",
  "Experiência em treinamento para corrida, desde iniciantes até maratonistas.",
  "Acompanho indivíduos com condições especiais, adaptando treinos para necessidades específicas.",
];

const manicureServices: string[] = [
  "Especialista em unhas de gel e alongamentos com acabamento natural e duradouro.",
  "Atuo com esmaltação clássica, trazendo sofisticação e elegância para todas as ocasiões.",
  "Experiência em nail art detalhada, com designs personalizados para cada cliente.",
  "Manicure com foco em cuidados das unhas, incluindo hidratação e fortalecimento.",
  "Especialista em unhas postiças e encapsuladas, para eventos especiais ou uso diário.",
  "Atendo clientes que buscam esmaltação em gel, garantindo brilho e resistência prolongados.",
  "Ofereço serviços de spa para mãos, com esfoliação, hidratação e massagem relaxante.",
  "Expert em cuidados de unhas para pessoas com diabetes, garantindo segurança e delicadeza.",
  "Realizo restauração e manutenção de unhas danificadas com técnicas avançadas.",
  "Personalizo cores e estilos de esmaltação para atender às preferências exclusivas de cada cliente.",
];

const cozinheiroServices: string[] = [
  "Especialista em culinária brasileira, com pratos tradicionais e regionais autênticos.",
  "Atuo com gastronomia internacional, incluindo pratos italianos, franceses e asiáticos.",
  "Experiência em cozinha saudável, criando refeições balanceadas e saborosas.",
  "Cozinheiro especializado em eventos, com cardápios personalizados para cada ocasião.",
  "Expert em confeitaria, desde bolos artísticos até sobremesas sofisticadas.",
  "Atendo clientes com restrições alimentares, oferecendo opções veganas, sem glúten ou sem lactose.",
  "Realizo preparação de refeições congeladas para o dia a dia, com foco em praticidade e qualidade.",
  "Especialista em churrascos, com cortes premium e acompanhamentos tradicionais.",
  "Planejo cardápios completos para festas, incluindo entrada, prato principal e sobremesa.",
  "Ofereço aulas particulares de culinária, ensinando técnicas e receitas de diferentes estilos.",
];

const eletricistaServices: string[] = [
  "Especialista em instalações elétricas residenciais, garantindo segurança e eficiência.",
  "Atuo com manutenção e reparos elétricos para sistemas de baixa e média tensão.",
  "Experiência em instalação de sistemas de energia solar e soluções sustentáveis.",
  "Cuidado em identificar e solucionar problemas elétricos, prevenindo curtos-circuitos e riscos.",
  "Especialista em automação residencial, integrando tecnologia para maior conforto e funcionalidade.",
  "Realizo instalação e manutenção de quadros de distribuição elétrica e circuitos internos.",
  "Expert em adequação de instalações elétricas para normas de segurança vigentes.",
  "Planejo projetos elétricos personalizados para obras novas ou reformas.",
  "Ofereço serviços de instalação de iluminação decorativa para ambientes internos e externos.",
  "Atuo com sistemas de segurança, incluindo instalação de câmeras, alarmes e interfone.",
];

const encanadorServices: string[] = [
  "Especialista em instalação e manutenção de sistemas hidráulicos residenciais.",
  "Realizo reparos em vazamentos, entupimentos e trocas de encanamento.",
  "Experiência na instalação de redes de água quente e fria.",
  "Trabalho com desentupimento de pias, vasos sanitários e ralos.",
  "Instalo e conserto registros, torneiras, chuveiros e caixas d'água.",
  "Executo projetos de encanamento para novas construções e reformas.",
  "Realizo testes de pressão e detecção de vazamentos não aparentes.",
  "Instalação e manutenção de sistemas de esgoto e redes de drenagem.",
  "Trabalho com solda em tubulações de cobre e conexões em PVC.",
  "Faço a troca e instalação de bombas d’água e aquecedores.",
];

const desenvolvedorServices: string[] = [
  "Especialista em desenvolvimento front-end, com foco em interfaces responsivas e design moderno.",
  "Atuo com desenvolvimento back-end, criando APIs robustas e escaláveis.",
  "Experiência em desenvolvimento full stack, integrando front-end e back-end com eficiência.",
  "Especialista em desenvolvimento de aplicativos móveis para Android e iOS.",
  "Realizo projetos de sistemas corporativos, com ênfase em gestão empresarial e automação de processos.",
  "Expert em otimização de desempenho de sistemas e redução de tempo de carregamento.",
  "Atendo demandas de desenvolvimento de e-commerce, incluindo integração com gateways de pagamento.",
  "Foco em soluções de inteligência artificial, machine learning e análise de dados.",
  "Planejo e desenvolvo soluções em nuvem, utilizando AWS, Azure ou Google Cloud.",
  "Ofereço consultoria em segurança cibernética, protegendo aplicações contra vulnerabilidades.",
];

const saudeServices: string[] = [
  "Consultas médicas presenciais e por telemedicina, com foco em diagnósticos precisos.",
  "Atendimento nutricional personalizado para perda de peso, ganho de massa muscular e reeducação alimentar.",
  "Acompanhamento psicológico para ansiedade, depressão e outros transtornos emocionais.",
  "Especialista em fisioterapia, com tratamentos para dores crônicas, reabilitação pós-cirúrgica e lesões esportivas.",
  "Realizo atendimento odontológico, incluindo limpeza, restaurações e tratamentos estéticos.",
  "Experiência em exames laboratoriais domiciliares, com coleta rápida e segura.",
  "Orientação e suporte em saúde preventiva, como vacinas, check-ups e rastreamento de doenças.",
  "Atendimento de emergência médica e primeiros socorros em domicílio.",
  "Ofereço serviços de acupuntura e técnicas de medicina integrativa para alívio de dores e equilíbrio energético.",
  "Aulas de pilates e treinamento funcional com foco na saúde e no bem-estar físico.",
];

export {
  jardineiroServices,
  marceneiroServices,
  pintorServices,
  DJServices,
  advocaciaServices,
  personalServices,
  manicureServices,
  cozinheiroServices,
  eletricistaServices,
  encanadorServices,
  desenvolvedorServices,
  saudeServices,
};
